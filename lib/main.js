        ...
        if(reg.test(url)){
            ...
            request.get(url,{timeout:self.timeout},function(err,response,body){
                if(err && err.code == 'ETIMEDOUT'){
                    ...
                    //返回成功状态并传递 视频url
                    resolve(data['data']['result']['mpath'][self.defintion])
                }
            })
        }else{
...
Crawler.prototype.showSearchList =function(url){
    ...
    html.then(function(body){
        ...
        parser.getInfo().forEach(function(e,i){
            console.log('---------------------------------------------------');
            console.log(colors.green('id: '),colors.bgRed(' '+ e.id + ' '));
            console.log(colors.green('title: '),e.title);
            console.log(colors.green('description: '),e.description);
            console.log(colors.green('url '),e.url);
        });
    },function(err){
